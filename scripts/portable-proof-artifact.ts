...
export async function verifyPortableProofArtifact(input: {
  ...
  if (!codexManifestBytes) {
    throw new Error(`Codex marketplace omitted ${codexManifestPath}.`);
  }
  ...
  await runSequentially(["codex", "cursor", "vscode"] as const, async (client) => {
    ...
    await runSequentially(Object.entries(receipt.coreFiles), async ([path, expectedDigest]) => {
      const relativePath = nodePath.relative(receipt.name, path);
      if (relativePath.startsWith(`..${nodePath.sep}`) || relativePath === "..") {
        throw new Error("Core receipt path escaped the plugin root.");
      }
      if (
        sha256(await regularFile(nodePath.join(installedRoot, relativePath))) !== expectedDigest
      ) {
    ...
    if (harness.client !== client) {
      throw new Error("Client adapter drifted.");
    }
    // oxlint-disable-next-line eslint/no-await-in-loop -- preserve intentional sequential control flow
