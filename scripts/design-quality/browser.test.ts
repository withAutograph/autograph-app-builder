...
describe("conservative design measurements", () => {
  it("keeps token references distinct from lookalikes", () => {
    expect(classifyStyle(["var(--color-text-primary)"], "rgb(41, 41, 41)", [])).toBe(
      "token-reference",
    );
    expect(classifyStyle(["#292929"], "rgb(41, 41, 41)", ["rgb(41, 41, 41)"])).toBe(
      "matching-literal",
    );
    expect(classifyStyle(["#123456"], "rgb(18, 52, 86)", [])).toBe("unmatched-literal");
    expect(classifyStyle(["var(--a)", "#292929"], "rgb(41, 41, 41)", [])).toBe("unassessed");
  ...
  it("attributes a stylesheet only when its source URL matches an explicit generated path", () => {
    expect(sourcePath("https://preview.example/app.css?token=secret#hash")).toBe("/app.css");
    expect(generatedSource("/workspace/apps/generated/app.css", ["apps/generated/app.css"])).toBe(
      true,
    );
    expect(
      generatedSource("/workspace/packages/design-systems/theme.css", ["apps/generated/app.css"]),
    ).toBe(false);
    expect(generatedSource(undefined, ["apps/generated/app.css"])).toBe(false);
    expect(arrustedSharedSource("/workspace/packages/design-systems/theme.css")).toBe(true);
