...
const receipt = JSON.parse(
  await readFile(join(releaseRoot, "release-receipt.json"), "utf-8")
);
if (
...
const discovery = JSON.parse(
  await readFile(join(releaseRoot, "mock/tools-list.json"), "utf-8")
);
...
for (const client of ["vscode", "cursor", "codex"] as const) {
  ...
  const harness = JSON.parse(
    await readFile(join(root, "client-harness.json"), "utf-8")
  );
  const installation = JSON.parse(
    await readFile(join(root, "installation-receipt.json"), "utf-8")
  );
  if (
