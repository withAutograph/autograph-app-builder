...
it.each(["docker", "process"])(
  "resolves %s tools through absolute mise when PATH cannot find mise",
  (backend) => {
...
`,
      ...
      if (backend === "docker") expect(calls).toContain("which docker");
      else expect(calls).not.toContain("docker");
      expect(calls).toContain(`--postgres-backend ${backend}`);
