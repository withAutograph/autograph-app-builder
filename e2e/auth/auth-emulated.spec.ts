...
test("passkey UI defaults off without a Vercel flag override", async ({ browser }) => {
  ...
  try {
    ...
    await expect(page.getByRole("button", { name: "Continue with Passkey" })).toHaveCount(0);
    await expect(page.getByRole("button", { name: "Continue with Vercel" })).toBeVisible();
...
test("a sign-in challenge failure stays local without invoking WebAuthn", async ({ page }) => {
...
for (const exceptionName of ["NotSupportedError", "SecurityError"] as const) {
  test(`a pre-assertion ${exceptionName} stays on Sign In without writes`, async ({ page }) => {
    ...
    page.on("request", (request) => {
      if (new URL(request.url()).pathname === "/api/auth/passkey/verify-authentication") {
...
test("passkey registration guards Sign Up and supports returning login", async ({
  ...
  try {
    ...
    page.on("request", (request) => {
      if (new URL(request.url()).pathname === "/api/auth/passkey/onboarding-context") {
        onboardingRequests += 1;
      }
    });

    await page.goto(
      `/auth/sign-up?callbackURL=${encodeURIComponent("/?source=signed-in#complete")}`,
    );
    ...
    await signOut(page);
    await page.goto(
      `/auth/sign-in?callbackURL=${encodeURIComponent("/?source=returning#complete")}`,
    );
    await page.getByRole("button", { name: "Continue with Passkey" }).click();
...
test("an interrupted passkey ceremony keeps the permanent Sign Up link", async ({ page }) => {
  await page.addInitScript(() => {
    const { credentials } = navigator;
    Object.defineProperty(navigator, "credentials", {
      configurable: true,
...
test("cancelled passkey registration stays on Sign Up without partial state", async ({ page }) => {
  await page.addInitScript(() => {
    const { credentials } = navigator;
    Object.defineProperty(navigator, "credentials", {
      configurable: true,
...
for (const contextFailure of [
  ...
  test(`a ${contextFailure.name} onboarding-context response stays on Sign Up without partial state`, async ({
    ...
    page.on("request", (request) => {
      if (new URL(request.url()).pathname === "/api/auth/passkey/generate-register-options") {
...
test("a session created after Sign Up renders blocks context issuance", async ({
  ...
  try {
    ...
    await staleSignUpPage.goto(
      `/auth/sign-up?callbackURL=${encodeURIComponent("/?source=context-race#complete")}`,
    );
    ...
    const responsePromise = staleSignUpPage.waitForResponse(
      (response) => new URL(response.url()).pathname === "/api/auth/passkey/onboarding-context",
    );
    await staleSignUpPage.getByRole("button", { name: "Continue with Passkey" }).click();
    ...
    await expect(staleSignUpPage).toHaveURL(/\/\?source=context-race#complete$/u);
    expect(await authCounts()).toEqual(baseline);
    expect(await authenticator.credentials()).toEqual(deviceCredentials);
  } finally {
...
test("a session created after context issuance blocks registration", async ({ context, page }) => {
  ...
  try {
    ...
    await page.goto(
      `/auth/sign-up?callbackURL=${encodeURIComponent("/?source=verification-race#complete")}`,
    );
    const clickPromise = page.getByRole("button", { name: "Continue with Passkey" }).click();
...
test("a final persistence failure rolls back registration state", async ({ context, page }) => {
  reportPasskeyFailures(page);
  const authenticator = await VirtualAuthenticator.create(context, page);
  const sql = postgres(databaseUrl, { max: 1 });
  try {
...
test("provider account supports multiple passkeys but retains its final passkey", async ({
  ...
  reportPasskeyFailures(page);
  let authenticator: VirtualAuthenticator | undefined = await VirtualAuthenticator.create(
    context,
    page,
  );
  try {
