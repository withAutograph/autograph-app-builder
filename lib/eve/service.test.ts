...
describe("local Eve acceptance", () => {
  ...
  it("keeps buffered progress readable while a clean response close tails a live turn", async () => {
    const tailReady = Promise.withResolvers<undefined>();
    const response = {
      // oxlint-disable-next-line eslint/require-await -- preserve Promise-returning test double
      cancel: vi.fn(async () => ({ status: "accepted" })),
      async *[Symbol.asyncIterator]() {
    ...
    tailReady.resolve(undefined as undefined);
    await vi.waitFor(async () => {
      await expect(
        service.get({ cursor: 0, limit: 100, sessionId: started.sessionId }),
      ).resolves.toMatchObject({ cursor: 2, status: "waiting" });
  ...
  it("preserves buffered settlement across a Next development module reload", async () => {
    ...
    await expect(
      ...
      events: expect.arrayContaining([
        ...
        expect.objectContaining({ status: "waiting", type: "status" }),
      ]),
      status: "waiting",
    });
  ...
  it("makes an active local turn resumable after its Eve child restarts", async () => {
    const oldResponseOpen = Promise.withResolvers<undefined>();
    const oldResponse = {
      // oxlint-disable-next-line eslint/require-await -- preserve Promise-returning test double
      cancel: vi.fn(async () => ({ status: "accepted" })),
      async *[Symbol.asyncIterator]() {
        ...
        await oldResponseOpen.promise;
      },
    };
    const resumedEvents = [{ data: {}, type: "session.waiting" }] as MessageStreamEvent[];
    ...
    const client = {
      sessions: {
        // oxlint-disable-next-line eslint/require-await -- preserve Promise-returning test double
        attach,
        create: vi.fn(() => Promise.resolve({ response: oldResponse, session })),
      } as never,
    };
    ...
    oldResponseOpen.resolve(undefined as undefined);
  });
  ...
  it("keeps a verified prototype on cursor-at-tail and accepted follow-ups", async () => {
    ...
    const service = createLocalEveSessionService({
      sessions: {
        // oxlint-disable-next-line eslint/require-await -- preserve Promise-returning test double
        attach: vi.fn(() => session),
        create: vi.fn(() => Promise.resolve({ response: response(events), session })),
      } as never,
    });
    const started = await service.start({
  ...
  it("returns one stable public handle without waiting for the active turn", async () => {
    ...
    // oxlint-disable-next-line eslint/require-await -- preserve Promise-returning test double
    const create = vi.fn(() => Promise.resolve({ response, session }));
    const attach = vi.fn(() => session);
    const service = createLocalEveSessionService({
  ...
  it("keeps get, send, and cancel independent while rejecting a non-outstanding response", async () => {
    ...
    const response = {
      // oxlint-disable-next-line eslint/require-await -- preserve Promise-returning test double
      cancel: vi.fn(async () => {
        cancelled.resolve(undefined as undefined);
    ...
    await vi.waitFor(async () => {
      await expect(
        ...
        events: expect.arrayContaining([
          expect.objectContaining({ status: "cancelled", type: "status" }),
        ]),
        status: "cancelled",
      });
    });
  ...
  it("rebinds follow-up and response streams at the exact buffered raw tail", async () => {
    ...
    const responded = stream([
      {
        data: {
          resolutions: ["request-source", "request-plan", "request-preview"].map((requestId) => ({
            ...
            outcome: "approved",
            requestId,
            response: { optionId: "approve", requestId },
          })),
          turnId: "turn-follow-up",
        },
        type: "input.resolved",
    ...
    const service = createLocalEveSessionService({
      sessions: {
        // oxlint-disable-next-line eslint/require-await -- preserve Promise-returning test double
        attach,
        create: vi.fn(() => Promise.resolve({ response: initial, session: created })),
      } as never,
    });
    const started = await service.start({
