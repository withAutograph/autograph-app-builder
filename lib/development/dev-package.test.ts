...
describe("development Codex package", () => {
  it("creates a stable loopback-only marketplace with exactly five public tools and no app surface", async () => {
    const root = await realpath(await mkdtemp(path.join(tmpdir(), "autograph-dev-package-")));
    try {
      ...
      const manifest = JSON.parse(
        await readFile(path.join(result.pluginRoot, ".codex-plugin/plugin.json"), "utf-8"),
      );
      const mcp = JSON.parse(await readFile(path.join(result.pluginRoot, ".mcp.json"), "utf-8"));
      const tools = JSON.parse(
        await readFile(path.join(result.pluginRoot, "tools-list.json"), "utf-8"),
      );
      expect(result.marketplaceRoot).toBe(path.join(root, "marketplace"));
  ...
  it("installs development guidance without enabling it globally", async () => {
    const codexHome = await mkdtemp(path.join(tmpdir(), "codex-scope-"));
    const commands: {
    ...
    try {
      await registerDevelopmentPackage({
        ...
        runner: async (args, options) => {
          commands.push({ allowFailure: options.allowFailure ?? false, args });
          ...
          if (args.includes("list")) {
            expect(await readFile(path.join(codexHome, "config.toml"), "utf-8")).toBe(
              'model = "gpt-6-astra"\n[plugins."app-builder@autograph-dev"]\nenabled = false\n[plugins."app-builder@autograph"]\nenabled = true\n',
            );
          }
          return {
