...
function unsigned(): PromotionReceiptUnsigned {
  return {
    ...
    package: {
      ...
      archive: "package/app-builder-0.2.3.tar.gz",
      archiveSha256: "4".repeat(64),
      marketplaceArchive: "package/app-builder-codex-marketplace-0.2.3.tar.gz",
...
async function candidate() {
  ...
  await writeFile(
    join(root, "package/app-builder-0.2.3.tar.gz"),
    packageArchive,
  );
  await writeFile(
    join(root, "package/app-builder-codex-marketplace-0.2.3.tar.gz"),
    marketplaceArchive,
  );
  ...
  const packageReceipt = {
    ...
    archive: {
      name: "app-builder-0.2.3.tar.gz",
      sha256: sha256(packageArchive),
    },
    codexMarketplaceArchive: {
      name: "app-builder-codex-marketplace-0.2.3.tar.gz",
...
describe("release promotion contract", () => {
  ...
  it("accepts exact candidate bytes and rejects every mutated binding", async () => {
    ...
    await writeFile(
      join(packageMutation, "package/app-builder-0.2.3.tar.gz"),
      "changed",
    );
    await expect(
