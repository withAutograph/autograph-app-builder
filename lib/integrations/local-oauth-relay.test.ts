...
describe("local Vercel OAuth relay", () => {
  it("accepts only signed, unexpired relay state", () => {
    ...
    const value = signLocalVercelRelay({ state: "a".repeat(32), configurationId: "icfg_1", teamId: "team_1", expiresAt: 2_000 }, secret);
    expect(verifyLocalVercelRelay(value, secret, 1_000)).toMatchObject({ teamId: "team_1" });
