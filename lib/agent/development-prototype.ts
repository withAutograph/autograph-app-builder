 ...
 * revisable prototype bundle from concise product choices. It is a fast path
 * for the supported patterns, not a universal layout; callers use the authored
 * bundle path when the product needs a different composition.
 */
export function developmentPrototypeBundle(
  ...
  return {
    ...
    indexHtml: bundle.indexHtml
      ...
      .replace(
        "</script>",
        ";document.querySelector('#action-cancel').addEventListener('click',()=>dialog.close())</script>",
      ),
  };
}
