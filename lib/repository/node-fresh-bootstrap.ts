...
const assertContainedStatePath = async (
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === "ENOENT" && leaf !== "directory") {
        return;
      }
      throw error;
    }
...
const acquireLease = async (
  ...
  holder.stdout.once("data", (chunk: string) => {
    ...
    if (chunk === "READY\n") {
      ...
      reject(new Error("Lease handshake failed."));
    }
  });
...
const quiesceAbandonedLease = async (
  ...
  holder.stdout.once("data", (chunk: string) => {
    ...
    if (chunk === "READY\n") {
      ...
      reject(new Error("Lease quiescence handshake failed."));
    }
  });
  const monitorExit = async () => {
    await exited;
...
const rawWorktreeManifest = async (
  ...
  const walk = async (relativePath: string): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      if (allowClaimMarker && path === proposal.claimMarkerName) {
        continue;
      }
...
const atomicPublish = async (
  ...
  try {
    ...
    if (proposal.destinationPrestate.kind === "absent") {
      publicationMode = "noreplace";
    } else if (hooks?.afterAtomicSwap === undefined) {
      publicationMode = "exchange";
    } else {
      publicationMode = "exchange-hold";
    }
    const result = spawnSync(
      capability.systemPython,
      [
...
export const readFreshBootstrapJournal = async (input: {
  ...
  if (
    !exactFreshBootstrapProposalMatch(proposalFromFreshBootstrapJournal(parsed), input.proposal)
  ) {
...
> => {
  ...
  try {
    ...
    if (destinationPublished) {
      reason = "publication-partial";
    } else if (stageCreated) {
      reason = "materialization-partial";
    } else {
