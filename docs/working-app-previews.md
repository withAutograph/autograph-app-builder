...
Focused tests cover actual gateway HTTP traffic, delayed activation, application authentication forwarding, real supervisor process-group shutdown, provider identity binding, cancellation, startup failures, and public receipt recovery. The 2026-09-14 public-entrypoint acceptance failed startup and recovery; see `docs/reports/self-reproduction/2026-09-14-working-preview-acceptance.md`. Startup timeouts now distinguish listener startup from application HTTP readiness and report only safe status observations. A new live acceptance is still required to establish that App Builder starts a real generated app and hands its working URL to the user. Do not count a direct infrastructure probe as self-reproduction evidence.
