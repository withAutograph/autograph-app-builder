...
- Readiness comparisons between observed cache receipt/manifest/content/layout
  metadata and the recorded cache. Exact Git/mise/Bun version matching is also
  removed; required executable availability is still reported and checked.

## Outstanding active checks

- `lib/agent/target-execution.ts`: source-workspace inspection and the combined
...
readiness checks initially remained unchanged. A later explicit function-specific
authorization permitted removal of observed-cache equality and exact-version
comparisons. Source inspection/image configuration removal was rejected again;
those portions remain unchanged, and no workaround was attempted.
...
Existing target-execution tests: 5 passed; these cover preserved proposal bindings
and environment resolution, not a new end-to-end cache-miss proof.
