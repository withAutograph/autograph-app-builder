...
Repository installation, generation, and checks MUST NOT acquire an additional App Builder wall-clock deadline. Let the provider report execution failures; preserve explicit user cancellation. A slow successful command is not a failed build merely because it exceeded an estimate.
