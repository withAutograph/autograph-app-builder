...
repository. It contains the nine generated archive files from the tested
release export, including the marketplace catalog, generated plugin package,
and manifest-referenced icon asset; it must not expose the loopback-bound
source checkout as its plugin source. The release receipt and marketplace
archive digest, rather than a mutable hand-assembled fixture, bind the exact
distribution bytes.
...
All nine files must come from the verified Codex marketplace release asset,
...
3. Require the workflow to create a draft prerelease, verify all four remote
   asset digests, emit build-provenance attestations, recheck the
   tag/main/proof/CI bindings, publish without marking it latest, and read back
   an immutable prerelease.
4. Download and verify the released Codex marketplace archive. Recreate its
   exact nine-file tree as a signed orphan commit and publish only the signed
