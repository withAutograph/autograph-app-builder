...
export const mergeSupplementaryObservation = (
...
const safeRelative = (pathValue: string) => {
  if (
    nodePath.isAbsolute(pathValue) ||
...
export const writeSupplementaryAssessment = async (input: {
  runDirectory: string;
  ...
  const output = nodePath.join(
    await realpath(nodePath.dirname(nodePath.resolve(input.outputDirectory))),
    nodePath.basename(input.outputDirectory),
  );
  ...
  if (!parityRoot || !reviewRoot) {
    throw new Error("Supplementary evidence roots are incomplete.");
  }
  ...
  const copy = async (root: string, artifact: string, prefix: string) => {
    ...
    try {
      ...
      if (
        nodePath.relative(root, source).startsWith("..") ||
        !metadata.isFile() ||
        metadata.size === 0
      ) {
  ...
  const evidence = parityEvidenceSchema.parse(
    JSON.parse(await readFile(nodePath.join(parityRoot, "parity-evidence.json"), "utf-8")),
  );
  const review = JSON.parse(
    await readFile(nodePath.join(reviewRoot, "observations.json"), "utf-8"),
  ) as {
  ...
  for (const side of sides) {
    evidence[side].observations = await Promise.all(
      evidence[side].observations.map((item) => rebase(item, parityRoot, "original")),
    );
    for (const raw of review.observations?.[side] ?? []) {
      ...
      const source = await rebase(normalized, reviewRoot, "source-review");
      ...
      if (index === -1) {
        evidence[side].observations.push(merged);
      } else {
        evidence[side].observations[index] = merged;
      }
    }
  }
  ...
  if (input.referenceCapturesDirectory) {
    ...
    const captureMetadata = JSON.parse(
      await readFile(nodePath.join(captureRoot, "capture-provenance.json"), "utf-8"),
    );
    for (const capture of captureMetadata.captures ?? []) {
      const artifact = safeRelative(nodePath.relative(captureRoot, await realpath(capture.path)));
      const reference = await copy(captureRoot, artifact, "reference-captures");
      const viewport = String(
        capture.viewport?.name ?? capture.name ?? nodePath.basename(artifact, ".png"),
      ).replace(/-0$/u, "");
  ...
  await writeFile(nodePath.join(output, "parity-evidence.json"), JSON.stringify(evidence, null, 2));
  await writeFile(nodePath.join(output, "report.json"), JSON.stringify(report, null, 2));
  ...
  await writeFile(nodePath.join(output, "report.md"), md);
