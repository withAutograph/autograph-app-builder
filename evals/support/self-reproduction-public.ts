...
export const makePublicTransport = async (
  ...
  const rpc = async (method: string, params: unknown, notification = false): Promise<unknown> => {
    ...
    if (response.headers.get("content-type")?.includes("text/event-stream")) {
      ...
      if (!reader) throw new Error("Missing public MCP response body");
      ...
      if (!found) throw new Error("Public MCP stream ended without response");
      envelope = found;
    } else envelope = (await response.json()) as typeof envelope;
    if (envelope.error) throw new Error(`Public MCP error: ${JSON.stringify(envelope.error)}`);
...
export const runPublicSession = async (options: {
  ...
  const respond = async () => {
    ...
    if (!pending || !state.session) throw new Error("Missing pending response session");
    accept(
